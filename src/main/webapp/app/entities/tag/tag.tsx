...
export const Tag = (props: ITagProps) => {
  ...
  return (
    <div>
      <h2 id="tag-heading">
        <Translate contentKey="noteyApp.tag.home.title">Tags</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="noteyApp.tag.home.createLabel">Create new Tag</Translate>
        </Link>
      </h2>
        ...
        {tagList && tagList.length > 0 ? (
            ...
            <tbody>
              {tagList.map((tag, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{tag.name}</td>
                  <td>{tag.assignedTo ? tag.assignedTo.login : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="noteyApp.tag.home.notFound">No Tags found</Translate>
            </div>
          )
        )}
      </div>
